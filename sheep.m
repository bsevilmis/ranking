function varargout = sheep(varargin)
%SHEEP M-file for sheep.fig
%      SHEEP, by itself, creates a new SHEEP or raises the existing
%      singleton*.
%
%      H = SHEEP returns the handle to a new SHEEP or the handle to
%      the existing singleton*.
%
%      SHEEP('Property','Value',...) creates a new SHEEP using the
%      given property value pairs. Unrecognized properties are passed via
%      varargin to sheep_OpeningFcn.  This calling syntax produces a
%      warning when there is an existing singleton*.
%
%      SHEEP('CALLBACK') and SHEEP('CALLBACK',hObject,...) call the
%      local function named CALLBACK in SHEEP.M with the given input
%      arguments.
%
...
% Edit the above text to modify the response to help sheep

% Last Modified by GUIDE v2.5 21-Apr-2015 20:33:05

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @sheep_OpeningFcn, ...
                   'gui_OutputFcn',  @sheep_OutputFcn, ...
                   'gui_LayoutFcn',  [], ...
                   'gui_Callback',   []);
...
% --- Executes just before sheep is made visible.
function sheep_OpeningFcn(hObject, eventdata, handles, varargin)
...
user.parts = keys(new{17});
user.ranking = zeros(1,numel(user.parts));
handles.user = user;

imshow(imread('./dataSets/trainval/guiVisuals/sheepModified.png'));
...
function varargout = sheep_OutputFcn(hObject, eventdata, handles)
...
if(~isempty(missingCategoryIndex))
    ...
    if(numel(unique(finalRanking)) ~= totalNumberOfParts)
        ...
        folderPrefix = './dataSets/trainval/humanRanks/sheep/%s';
