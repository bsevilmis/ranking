function varargout = catt(varargin)
%CATT M-file for catt.fig
%      CATT, by itself, creates a new CATT or raises the existing
%      singleton*.
%
%      H = CATT returns the handle to a new CATT or the handle to
%      the existing singleton*.
%
%      CATT('Property','Value',...) creates a new CATT using the
%      given property value pairs. Unrecognized properties are passed via
%      varargin to catt_OpeningFcn.  This calling syntax produces a
%      warning when there is an existing singleton*.
%
%      CATT('CALLBACK') and CATT('CALLBACK',hObject,...) call the
%      local function named CALLBACK in CATT.M with the given input
%      arguments.
%
...
% Edit the above text to modify the response to help catt

% Last Modified by GUIDE v2.5 21-Apr-2015 18:01:12

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @catt_OpeningFcn, ...
                   'gui_OutputFcn',  @catt_OutputFcn, ...
                   'gui_LayoutFcn',  [], ...
                   'gui_Callback',   []);
...
% --- Executes just before catt is made visible.
function catt_OpeningFcn(hObject, eventdata, handles, varargin)
...
user.parts = keys(new{8});
user.ranking = zeros(1,numel(user.parts));
handles.user = user;

imshow(imread('./dataSets/trainval/guiVisuals/catModified.png'));
...
% UIWAIT makes catt wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = catt_OutputFcn(hObject, eventdata, handles)
...
if(~isempty(missingCategoryIndex))
    ...
    if(numel(unique(finalRanking)) ~= totalNumberOfParts)
        ...
        folderPrefix = './dataSets/trainval/humanRanks/cat/%s';
