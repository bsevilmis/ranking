function varargout = cow(varargin)
%COW M-file for cow.fig
%      COW, by itself, creates a new COW or raises the existing
%      singleton*.
%
%      H = COW returns the handle to a new COW or the handle to
%      the existing singleton*.
%
%      COW('Property','Value',...) creates a new COW using the
%      given property value pairs. Unrecognized properties are passed via
%      varargin to cow_OpeningFcn.  This calling syntax produces a
%      warning when there is an existing singleton*.
%
%      COW('CALLBACK') and COW('CALLBACK',hObject,...) call the
%      local function named CALLBACK in COW.M with the given input
%      arguments.
%
...
% Edit the above text to modify the response to help cow

% Last Modified by GUIDE v2.5 21-Apr-2015 17:59:23

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @cow_OpeningFcn, ...
                   'gui_OutputFcn',  @cow_OutputFcn, ...
                   'gui_LayoutFcn',  [], ...
                   'gui_Callback',   []);
...
% --- Executes just before cow is made visible.
function cow_OpeningFcn(hObject, eventdata, handles, varargin)
...
user.parts = keys(new{10});
user.ranking = zeros(1,numel(user.parts));
handles.user = user;

imshow(imread('./dataSets/trainval/guiVisuals/cowModified.png'));
...
% UIWAIT makes cow wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = cow_OutputFcn(hObject, eventdata, handles)
...
if(~isempty(missingCategoryIndex))
    ...
    if(numel(unique(finalRanking)) ~= totalNumberOfParts)
        ...
        folderPrefix = './dataSets/trainval/humanRanks/cow/%s';
